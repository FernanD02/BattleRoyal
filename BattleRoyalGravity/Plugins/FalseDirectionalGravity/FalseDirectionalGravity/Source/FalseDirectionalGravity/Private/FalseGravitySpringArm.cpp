...
#include "FalseGravitySpringArm.h"
...
//////////////////////////////////////////////////////////////////////////
// UFalseGravitySpringArm

const FName UFalseGravitySpringArm::SocketName(TEXT("SpringEndpoint"));

UFalseGravitySpringArm::UFalseGravitySpringArm(const FObjectInitializer& ObjectInitializer)
...
FRotator UFalseGravitySpringArm::GetTargetRotation() const
{
	...
	if (bUsePawnControlRotation)
	{
		if (APawn* OwningPawn = Cast<APawn>(GetOwner()))
		{
			...
			if (AFalseGravityCharacter* CurrentFalseCharacter = Cast<AFalseGravityCharacter>(OwningPawn))
			{
				//PawnViewRotation.Pitch = FMath::Clamp(PawnViewRotation.Pitch, CurrentFalseCharacter->MinGravityCameraPitch, CurrentFalseCharacter->MaxGravityCameraPitch);
				float Pitch = PawnViewRotation.Pitch;
				//UE_LOG(LogTemp, Error, TEXT("PITCH BEFORE = %f"), Pitch);
				if (Pitch < 180.f)
				{
					Pitch = FMath::Min(Pitch, -1.f * CurrentFalseCharacter->MinGravityCameraPitch);
				}
				else
				{
					Pitch = FMath::Max(Pitch, 360.f - CurrentFalseCharacter->MaxGravityCameraPitch);
				}
				PawnViewRotation.Pitch = Pitch;
				//UE_LOG(LogTemp, Error, TEXT("PITCH AFTER = %f"), Pitch);
			}

			FQuat PawnViewQuat = PawnViewRotation.Quaternion();

			FQuat QuatToFix = DesiredRot.Quaternion();

			FQuat Delta = FQuat::FindBetweenVectors(FVector(0.f, 0.f, 1.f), QuatToFix.GetAxisZ());

			FQuat Pitch = Delta * PawnViewQuat;

			DesiredRot = Pitch.Rotator();
	...
	//LastQuat = DesiredRot.Quaternion();

	return DesiredRot;
}

void UFalseGravitySpringArm::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
...
FVector UFalseGravitySpringArm::BlendLocations(const FVector& DesiredArmLocation, const FVector& TraceHitLocation, bool bHitSomething, float DeltaTime)
{
	return bHitSomething ? TraceHitLocation : DesiredArmLocation;
}

void UFalseGravitySpringArm::ApplyWorldOffset(const FVector & InOffset, bool bWorldShift)
{
...
void UFalseGravitySpringArm::OnRegister()
{
	Super::OnRegister();
...
void UFalseGravitySpringArm::PostLoad()
{
	Super::PostLoad();
}

void UFalseGravitySpringArm::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);
	UpdateDesiredArmLocation(bDoCollisionTest, bEnableCameraLag, bEnableCameraRotationLag, DeltaTime);
}

FTransform UFalseGravitySpringArm::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const
{
...
bool UFalseGravitySpringArm::HasAnySockets() const
{
	return true;
}

void UFalseGravitySpringArm::QuerySupportedSockets(TArray<FComponentSocketDescription>& OutSockets) const
{
	new (OutSockets) FComponentSocketDescription(SocketName, EComponentSocketType::Socket);
}

FVector UFalseGravitySpringArm::GetUnfixedCameraPosition() const
{
	return UnfixedCameraPosition;
}

bool UFalseGravitySpringArm::IsCollisionFixApplied() const
{
