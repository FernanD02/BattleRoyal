...
UFalseGravityMovementComponent::UFalseGravityMovementComponent()
{
	bFallingRemovesSpeedZ = true;
	bIgnoreBaseRollMove = true;
	CustomGravityDirection = FVector(0.f,0.f,-1.f);
	bGravityEnabled = true;

	if (AFalseGravityCharacter* Char = Cast<AFalseGravityCharacter>(CharacterOwner))
	{
		//MaxNewGravityDegrees = Char->GetMaxNewGravityDegrees();
		//MaxDistanceToFloor = Char->GetMaxDistanceToFloor();
		//GravityInterpolationSpeed = Char->GetGravityInterpolationSpeed();
		//GravityFallingInterpolationSpeed = Char->GetGravityFallingInterpolationSpeed();
		//UE_LOG(LogTemp, Warning, TEXT("UFalseGravityMovementComponent: Char valid"))
	}
}

FVector UFalseGravityMovementComponent::GetVolumeGravityDirection() const
{
	return FVector(0.f, 0.f, -1.f);
	//UE_LOG(LogTemp, Log, TEXT("UFalseGravityMovementComponent: GetVolumeGravityDirection"))
	TArray<AActor*> Actors;
	CharacterOwner->GetOverlappingActors(Actors, AFalseGravityVolumeBase::StaticClass());
	if (Actors.Num() > 0)
	{
		//UE_LOG(LogTemp, Log, TEXT("UFalseGravityMovementComponent: GetVolumeGravityDirection find overlap Actor"))
		TArray<AFalseGravityVolumeBase*> Volumes;
		for (AActor* TempActor : Actors)
		{
			if (AFalseGravityVolumeBase* TempGravityVolume = Cast<AFalseGravityVolumeBase>(TempActor))
			{
				Volumes.Add(TempGravityVolume);
			}
		}

		if (Volumes.Num() > 0)
		{
			//UE_LOG(LogTemp, Log, TEXT("UFalseGravityMovementComponent: GetVolumeGravityDirection find overlap volume"))
			AFalseGravityVolumeBase* TargetGravityVolume = Volumes[0];
			for (int32 i = 1; i < Volumes.Num(); i++)
			{
				if (Volumes[i]->Priority >= TargetGravityVolume->Priority)
				{
					TargetGravityVolume = Volumes[i];
				}
			}
			return TargetGravityVolume->GetGravity(GetActorLocation());
		}
	}
	//UE_LOG(LogTemp, Log, TEXT("FalseGravityMovementComponent: GetVolumeGravityDirection return default"))
	return FVector(0.f, 0.f, -1.f);
}

FVector UFalseGravityMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
	if (!bGravityEnabled)
		return FVector(0.f, 0.f, -1.f);
...
void UFalseGravityMovementComponent::SetGravityDirection(FVector NewGravityDirection)
{
	CustomGravityDirection = NewGravityDirection.GetSafeNormal();
}

void UFalseGravityMovementComponent::PhysFlying(float deltaTime, int32 Iterations)
{
...
float UFalseGravityMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
...
void UFalseGravityMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	if (!HasValidData())
	{
		return;
...
void UFalseGravityMovementComponent::PerformMovement(float DeltaTime)
{
	if (!HasValidData())
	{
		return;
...
void UFalseGravityMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice /*= 0.f*/, const FVector& MoveDelta /*= FVector::ZeroVector*/)
{
	if (CharacterOwner)
	{
...
void UFalseGravityMovementComponent::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
void UFalseGravityMovementComponent::MaintainHorizontalGroundVelocity()
{
	return;
...
float UFalseGravityMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void UFalseGravityMovementComponent::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
...
void UFalseGravityMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
...
FVector UFalseGravityMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const	// OK
{
...
void UFalseGravityMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult /*= NULL*/)
{
	if (!CurrentFloor.IsWalkableFloor())
	{
		return;
...
bool UFalseGravityMovementComponent::IsFalling() const
{
	return (MovementMode == MOVE_Falling) && UpdatedComponent;
}

void UFalseGravityMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	if (!bGravityEnabled)
	{
	...
	FCollisionQueryParams TraceParams = FCollisionQueryParams(FName(TEXT("GravityTrace")), true, CharacterOwner);
	//TraceParams.bTraceComplex = true;
	//TraceParams.AddIgnoredActor(CharacterOwner);

	FVector StartTrace = CharacterLocation + CustomGravityDirection * (HalfHeight - 10.f);
	FVector EndTrace = CharacterLocation + CustomGravityDirection * (HalfHeight + (bUseCustomGravityTraceLength ? MaxDistanceToFloor : (GetMaxJumpHeight() + 5.f)));
	FVector EndTraceOnFloor = CharacterLocation + CustomGravityDirection * (HalfHeight + 15.f);

	//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: StartTrace = %f : %f : %f"), StartTrace.X, StartTrace.Y, StartTrace.Z)
	//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: EndTrace = %f : %f : %f"), EndTrace.X, EndTrace.Y, EndTrace.Z)

	bool bExistFloor = GetWorld()->LineTraceSingleByChannel(OutHit, StartTrace, EndTrace, ECC_Visibility, TraceParams);
	bOnFloor = GetWorld()->LineTraceSingleByChannel(OutHitOnFloor, StartTrace, EndTraceOnFloor, ECC_Visibility, TraceParams);
	
	//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: bExistFloor = %s"), *FString(bExistFloor ? TEXT("true") : TEXT("false")));

	if (bDebugTraceEnabled)
	{
		//UKismetSystemLibrary::LineTraceSingle(this, StartTrace, EndTrace, ETraceTypeQuery::TraceTypeQuery1, true, CharacterOwner, EDrawDebugTrace::Type::ForOneFrame, OutHit, true);
	...
	//bExistFloot = OutHit.bBlockingHit;
	FVector NewGravity = bExistFloor ? OutHit.ImpactNormal * -1.f : (bResetGravityOnTraceFail ? FVector(0.f,0.f,-1.f) : CustomGravityDirection);

	//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: bExistFloor = %s"), *FString(bExistFloor ? "true" : "false"))

	//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: NewGravity = %f : %f : %f"), NewGravity.X, NewGravity.Y, NewGravity.Z)
	//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: CustomGravityDirection = %f : %f : %f"), CustomGravityDirection.X, CustomGravityDirection.Y, CustomGravityDirection.Z)

	if (!NewGravity.Equals(CustomGravityDirection, 0.0001))
	{
		//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: NewGravity.Equals"))
		if (bExistFloor)
		{
			const float Degrees = (180.f) / PI * FMath::Acos(FVector::DotProduct(NewGravity, CustomGravityDirection));
			if (Degrees <= MaxNewGravityDegrees)
			{
				//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: MaxDegrees: %f, Degrees: %f"), Degrees, MaxNewGravityDegrees);
				CustomGravityDirection = FMath::VInterpTo(CustomGravityDirection, NewGravity, DeltaTime, GravityInterpolationSpeed);
				//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: GravityInterpolationSpeed = %f"), GravityInterpolationSpeed)
			}
		}
		else
		{
			//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: bExistFloor false"))
			CustomGravityDirection = FMath::VInterpTo(CustomGravityDirection, NewGravity, DeltaTime, GravityFallingInterpolationSpeed);
			//UE_LOG(LogTemp, Warning, TEXT("SimulateMovement: GravityFallingInterpolationSpeed = %f"), GravityFallingInterpolationSpeed)
		}
	}
...
void UFalseGravityMovementComponent::SimulateMovement(float DeltaTime)
{
...
FVector UFalseGravityMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
FVector UFalseGravityMovementComponent::ScaleInputAcceleration(const FVector& InputAcceleration) const
{
	return GetMaxAcceleration() * GetClampedToMaxSizePrecise(InputAcceleration, 1.0f);
}

FVector UFalseGravityMovementComponent::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaTime, const FVector& CurrentVelocity) const
{
...
void UFalseGravityMovementComponent::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc)
{
	const float DesiredDist = Delta.Size();
...
void UFalseGravityMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
FVector UFalseGravityMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
	const FVector GravityDirection = GetGravityDirection();
...
FVector UFalseGravityMovementComponent::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const
{
...
void UFalseGravityMovementComponent::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
bool UFalseGravityMovementComponent::DoJump(bool bReplayingMoves)
{
...
FVector UFalseGravityMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UFalseGravityMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
void UFalseGravityMovementComponent::FindFloor(const FVector& CapsuleLocation, FFindFloorResult& OutFloorResult, bool bZeroDelta, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	...
	{
		UFalseGravityMovementComponent* MutableThis = const_cast<UFalseGravityMovementComponent*>(this);
...
void UFalseGravityMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
bool UFalseGravityMovementComponent::FloorSweepTest(struct FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool UFalseGravityMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool UFalseGravityMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetCapsuleAxisZ();
...
bool UFalseGravityMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius /*= true*/) const
{
...
bool UFalseGravityMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool UFalseGravityMovementComponent::CanStepUp(const FHitResult& Hit) const
{
...
bool UFalseGravityMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult &Hit, struct UCharacterMovementComponent::FStepDownResult* OutStepDownResult /*= NULL*/)
{
...
void UFalseGravityMovementComponent::SetMovementMode(EMovementMode NewMovementMode, uint8 NewCustomMode /*= 0*/)
{
...
void UFalseGravityMovementComponent::SetDefaultMovementMode()
{
...
void UFalseGravityMovementComponent::AdjustFloorHeight()
{
...
bool UFalseGravityMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
...
FVector UFalseGravityMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
void UFalseGravityMovementComponent::StartNewPhysics(float deltaTime, int32 Iterations)
{
...
void UFalseGravityMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
...
void UFalseGravityMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
bool UFalseGravityMovementComponent::IsWalkable(const FHitResult& Hit) const
{
	if (!Hit.IsValidBlockingHit())
	{
...
void UFalseGravityMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	OutFloorResult.Clear();
...
bool UFalseGravityMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const FVector CapsuleDown = GetCapsuleAxisZ() * -1.0f;
...
FVector UFalseGravityMovementComponent::GetGravity() const
{
	if (!CustomGravityDirection.IsZero())
	{
...
FVector UFalseGravityMovementComponent::GetComponentDesiredAxisZ() const
{
	return GetGravityDirection(true) * -1.0f;
}

void UFalseGravityMovementComponent::UpdateComponentRotation()
{
	if (!UpdatedComponent)
	{
		return;
...
FORCEINLINE FQuat UFalseGravityMovementComponent::GetCapsuleRotation() const
{
	return UpdatedComponent->GetComponentQuat();
}

FORCEINLINE FVector UFalseGravityMovementComponent::GetCapsuleAxisX() const
{
...
FORCEINLINE FVector UFalseGravityMovementComponent::GetCapsuleAxisZ() const
{
...
FORCEINLINE FVector UFalseGravityMovementComponent::GetSafeNormalPrecise(const FVector& V)
{
	const float VSq = V.SizeSquared();
	if (VSq < SMALL_NUMBER)
	{
...
bool UFalseGravityMovementComponent::IsWithinEdgeToleranceNew(const FVector& CapsuleLocation, const FVector& CapsuleDown, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const float DistFromCenterSq = (CapsuleLocation + CapsuleDown * ((TestImpactPoint - CapsuleLocation) | CapsuleDown) - TestImpactPoint).SizeSquared();
	const float ReducedRadiusSq = FMath::Square(FMath::Max(KINDA_SMALL_NUMBER, CapsuleRadius - SWEEP_EDGE_REJECT_DISTANCE));
